...
| MCP tools | 24 | `src/app/api/mcp/route.ts` · registry `ar.com.consignatarias/cattle-market` v1.4.0 |
...
- **MCP surfaces stay in sync.** Any change to MCP tools (add/remove/gating/pricing) must update ALL FOUR surfaces in the same PR: (1) `src/app/api/mcp/route.ts` — tool descriptions + the `instructions` string in `initialize`, (2) `/mcp` page — `TOOLS` array + footnote copy, (3) `src/app/llms.txt/route.ts` — tool list + access/pricing paragraph, (4) `src/app/llms-full.txt/route.ts` — the MCP callout. x402 pricing (US$0.05 tropa / US$0.10 arrendamiento / **US$0.25 serie INMAG completa** / PRO al blue) lives in the endpoint specs under `src/app/api/x402/` — if it changes, the copy in all four surfaces changes with it. **El único techo del server MCP es la profundidad histórica**, y son dos: `get_inmag_historico` es gratis hasta 365 días de ventana (y cualquier fecha puntual, a cualquier año), y `get_vr_historico` hasta 30 días — la ventana que ya está publicada en `/mercado` y `/vr`, así que el techo no cierra nada que estuviera abierto. Los dos RECORTAN y lo declaran, nunca niegan. La doctrina y los números que la fundan viven en `src/lib/inmag-historico.ts`.
