...
**Three product lines** (PRO Usuario · PRO Consignataria · Enterprise API · Self-serve) • **30+ API endpoints with auth + quotas** • **Lote-level transactional data** (haciinfo000007, pipeline recovered v1.16) • **MAG deep integration (16 sub-categorías + 11 años de INMAG)** • **USD blue desde 2011** • **347 remates** • **80 consignatarias canonical** (top 20 with referente / bio / región operativa / 9 con foto pública) • **64 MAG consignatarios** • **1092 frigoríficos (860 SENASA-verified + 232 sin verificación)** • **Productor reviews** (submission + admin moderation queue, v1.17) • **SENASA habilitados cross-reference** (monthly snapshot · free badge + PRO detail) • **12 provincias** • **30 YouTube channels mapped (85% upcoming-stream coverage)** • **1062 sitemap URLs** • **Consignatario as protagonist** (Quién Opera + Historial Verificable + Reseñas per profile · home regional grid · v1.16–v1.17) • **Schema.org Breadcrumb · Event · LocalBusiness · ItemList · Speakable · Dataset · FAQPage · VideoObject** • **llms.txt for AI crawlers** • **Programmatic OG per auction** • **7 daily-rebuilt price landings + `/precios` hub** • **Reports system** • **🔴 En Vivo with channel-based fallback** • **Observability (`/admin/ops`)** • **4 reusable internal audits** (data integrity · API health · link graph · content quality) • **P0/P1 security closed**

**Current:** v1.17.0 (2026-05-19) — see [CHANGELOG.md](./CHANGELOG.md) for the v1.16→v1.17 consignatario-as-protagonist arc (persona schema + Quién Opera + Historial Verificable + Productor Reviews with admin moderation + home repositioned around a regional grid; MAG lots pipeline recovery; SENASA habilitados verification with PRO-gated detail; frigorificos directory 364 → 1092; audit triplet; 6 link-graph bug fixes; archived-remate 301 redirects; MAG cron schedule fix; top-20 persona seed with 9 referente photos), and the v1.13.1 → v1.15.0 release train (SEO sprint with title-template fix, slug-hang fix, public API auth gate, observability foundation, 2 P0 + 5 P1 security findings closed, repo consistency audit). **No Enterprise API contract changes in v1.16 or v1.17** — `/api/precios`, `/api/lots`, `/api/index/memola` keep their shapes; `/api/lots` starts returning real lot-level data on the first Tue/Wed/Fri after deploy once the recovered pipeline first fires.
