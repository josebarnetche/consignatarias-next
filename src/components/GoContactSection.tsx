...
import SmartWhatsAppCTA from './SmartWhatsAppCTA'

interface GoContactSectionProps {
  slug: string
  displayName: string
...
/**
 * Contact section for /go landing pages.
 * Uses SmartWhatsAppCTA for lead capture before WhatsApp.
 */
export default function GoContactSection({
  slug,
  displayName,
  whatsapp,
  phone,
  isPro,
}: GoContactSectionProps) {
  return (
    <>
      {/* Contact CTAs */}
      <div className="space-y-3 mb-8">
        {whatsapp && (
          <SmartWhatsAppCTA
            slug={slug}
            consignatariaName={displayName}
            whatsapp={whatsapp}
            message="Hola, vi su perfil en consignatarias.com.ar y me gustaría consultar sobre sus remates."
            source="go_landing"
            skipCapture={!isPro} // Only show lead capture for PRO consignatarias
        ...
        <Link
          href={`/consignatarias/${slug}?utm_source=go&utm_medium=landing`}
