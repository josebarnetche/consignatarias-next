            ...
            {(profile.whatsapp || profile.phone || profile.email || profile.website) ? (
              <div className="flex flex-col gap-1.5">
                {profile.whatsapp && (
                  ...
                  <a href={`https://wa.me/${profile.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" onClick={() => { trackWhatsAppClick({ url: profile.whatsapp || '', slug: profile.canonicalSlug, source: 'profile' }); requestAccountNudge({ reason: 'contact_reveal', strength: 'strong' }) }} className="inline-flex items-center justify-center gap-2 rounded-terminal bg-emerald-500 hover:bg-emerald-400 px-3 py-2 text-sm font-semibold text-white transition-colors">
                ...
                {profile.phone && (
                  // tel: link (click-to-call en mobile) + value-event contact_phone (w6).
                  // Antes era texto plano → el índice de valor estaba ciego al lead telefónico.
                  <a href={`tel:${profile.phone.replace(/[^\d+]/g, '')}`} onClick={() => trackValueEvent('contact_phone', { entityType: 'consignataria', entitySlug: profile.canonicalSlug })} className="text-xxs font-terminal text-zinc-300 hover:text-accent-bright transition-colors"><span className="text-zinc-500 uppercase">Tel:</span> {profile.phone}</a>
                )}
                {profile.email && (
                  <a href={`mailto:${profile.email}`} onClick={() => trackValueEvent('contact_email', { entityType: 'consignataria', entitySlug: profile.canonicalSlug })} className="text-xxs text-accent hover:text-accent-bright font-terminal transition-colors truncate">{profile.email}</a>
                )}
                {profile.website && (
                  <a href={normalizeUrl(profile.website) || '#'} target="_blank" rel="noopener noreferrer" onClick={() => { trackOutboundClick(profile.website || '', 'website'); trackValueEvent('contact_web', { entityType: 'consignataria', entitySlug: profile.canonicalSlug }); requestAccountNudge({ reason: 'contact_reveal', strength: 'strong' }) }} className="text-xxs text-accent hover:text-accent-bright font-terminal transition-colors truncate">
