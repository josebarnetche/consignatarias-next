...
Todos los tools son públicos: el número del día, los precios por firma, los remates, el directorio, la sanidad y el valor de la tierra son gratis y sin cupo, y así van a seguir. Lo que se paga es la PROFUNDIDAD, no el acceso: la serie de dispersión (\`get_vr_historico\`) más allá de 30 días de ventana —los últimos 30 son los que ya están publicados en /mercado y /vr, así que gratis— con API key Enterprise; la serie histórica del INMAG más allá de 365 días de ventana (US\$0,25 en \`/api/x402/inmag-historico\`; cualquier fecha puntual sigue siendo gratis) y las valuaciones una vez agotado el cupo diario — la misma consulta se paga por request en USDC real (red Base) vía el protocolo x402, US\$0,05 en \`/api/x402/valuar-tropa\`, US\$0,10 en \`/api/x402/valuar-arrendamiento\`; el 402 trae las instrucciones de pago (scheme "exact", header X-PAYMENT). El PRO Consignataria también se paga en USDC en \`/api/x402/pro\`. API key Enterprise (\`Authorization: Bearer cnsg_live_...\`) para alertas ilimitadas, históricos bulk y soporte.
